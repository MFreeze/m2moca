...
\begin{proof}
    \begin{itemize}
        ...
        \item[$a \Rightarrow b$] Supposons maintenant que l'algorithme retourne faux alors il existe
            ...
            Un tel échec ne peut se produire que lors de l'examen de sacs fusions, en effet, sacs
            feuilles fournissant une solution triviale, permettent l'initialisation des solutions
            partielles. Tant que des sacs ajouts ou suppression sont étudiés, il existera
            constamment une solution partielle calculée à partir de la solution triviale obtenue
            pour un sac feuille.

            L'échec se produit donc pendant l'examen d'un sac fusion, ce qui implique qu'il n'existe
            aucune paire de solutions compatibles dans les fils $t_1$ et $t_2$ de $t$. Or $X_t$ est
            un séparateur, 
% }}}
%% {{{ COMPLEXITE
%% }}}
